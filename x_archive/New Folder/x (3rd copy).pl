...
for my $i (0..$second_col-1) {
    push @second_col, $temp_col[$i];
}

for my $i ($second_col..($second_col + $third_col)-1) {
    push @third_col, $temp_col[$i];
}

for my $i (( $second_col + $third_col )..($second_col + $third_col + $fourth_col)-1) {
...
for my $i (1..3) {
    ...
    if ($shortened_clue_list[$first_lines[$i]] !~ m{^\s*\d+\.\s}ixms) {
        print "ORPHAN ... \n";
        $shortened_clue_list[$first_lines[$i] - 1] .= " ->";
    }
}
print "===========================\n";
