...
sub carve_up_long_string {
    ...
    for ( my $i = $str_font; $i > $minimum_font; $i-- ) {
        ...
        for ( my $i = 0; $i < $num_words; $i++ ) {
            ...
            #print $temp_str, "<<<<<<<.................\n" or croak;
            if ( $temp_str =~ m/DOWN/ixms ) {
                push @temp_words, q{[[BLANK]]};
            }
            $temp_str =~ s/999\. //ixms;

            if ($DEBUG) {
                print int( $txt->advancewidth($temp_str) ), q{ -- >>}, $temp_str, qq{<<\n} or croak;
            }
            my $new_temp_str;
...
if ($DEBUG) {
    ...
    print join ',', @across or croak;
    print q{\n\n} or croak;
    print "DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD\n" or croak;
    print join ',', @down or croak;
    print q{\n\n} or croak;
}
...
for ( my $col = 0; $col < 4; $col++ ) {
    ...
    for ( my $i = $first_line; $i < $last_line; $i++ ) {
        ...
        if ( $top_line == 1 and $output =~ m/BLANK/ixms ) {
            $last_line++;
            $first_lines[2]++;
            $first_lines[3]++;
            $columns[2]--;
        ...
        if ($three_digit_across) {
            ...
            if ( $output =~ m/[[BLANK]]/ixms ) {
                $output = q{  };
            }

            if ( $output =~ m/ACROSS/ixms ) {
                $txt->font( $font{'Times'}{'Bold'}, $proper_font_size + 2 );
                $output = '::ACROSS:::::::';
            }

            if ( $output =~ m/DOWN/ixms ) {
                $txt->font( $font{'Times'}{'Bold'}, $proper_font_size + 2 );
                $output = '::DOWN:::::::';
            }
