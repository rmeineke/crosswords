...
#here is the code for writing the numbers
#and boxes into the grid......
my $sq_cnt = 0;
for ( my $i = 0; $i < $height - 1; $i++ ) {
    
    if ( grep( /^$sq_cnt$/,@circles) ) {
        print "$sq_cnt -- this sq needs circling\n";
    }
    
    $y = $st_y - ( $i * $sq_size ) - $sq_size;
    for ( my $j = 0; $j < $width - 1; $j++ ) {
        $x = $st_x + ( $j * $sq_size );
