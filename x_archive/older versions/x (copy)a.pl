...
sub carve_up_long_string {
    ...
    for ($i = 14; $i > 10; $i--) {
        ...
        for (my $i = 0; $i < $num_words; $i++) {
            ...
            if ( int( $txt->advancewidth($temp_str) ) > 125 ) {
                $i--;
                push(@temp_words, $str);
                $temp_str = q{};
            }
        ...
        my $height = ($num_strings * $i) + 2;
        print $height, " <<<<<<<<<<<<<<<<<<<<<<< \n";
        last if $height < $max_height;
        
    }
    foreach my $w (@temp_words) {
        #print int( $txt->advancewidth($w)), "\t == $w\n";
    }
    $pdf->end;
    return ($i, @temp_words);    
}
...
my ($font_size, @shortened_clue_list) = carve_up_long_string($long_str, $day);
